...
def show_clustering_page(df_raw):
    ...
    vars_sauts_puissance = [
        "CMJ 2JB", "Peak Force CMJ", "RFD CMJ", "RSI", "Drop jump",
    ...
    with tab1:
        ...
        # --- FORCER LA CONVERSION DES DONNÉES EN NOMBRES ---
        ...
        for col in df_filtered.columns:
            if col not in colonnes_texte:
                # S'il y a des virgules (type européen), on les remplace par des points
                try:
                    df_filtered[col] = df_filtered[col].astype(str).str.replace(',', '.')
                except:
                    pass
                # On force la conversion. Les caractères bizarres ("-", "NA") deviendront des vides mathématiques (NaN)
    ...
    with tab2:
        ...
        # Le curseur de tolérance
        seuil_na = st.slider("Tolérance aux données manquantes par test (%)", 10, 100, 95, help="Si un test possède plus de X% de cases vides, il est automatiquement exclu du calcul.")
        
        st.markdown("---")
        st.markdown("#### Ajustement des variables")
        
        # Liste stricte des colonnes à NE JAMAIS proposer dans le menu déroulant
        colonnes_interdites = [
            'Joueur', 'Player ID', 'N° GPS', 'Latéralité', 'Poste', 'Position', 
            'Date de Naissance', 'DT exact', 'Session exact', 'Session', 'Equipe', 'Numero'
        ]
        
        # On crée la liste de toutes les variables autorisées
        available_vars = [
        ...
        # Les variables cochées par défaut viennent de l'onglet 1
        default_actives = [v for v in target_vars if v in available_vars]

        # Le fameux menu déroulant
        vars_pca_actives = st.multiselect(
            "Ajoutez ou retirez manuellement des variables de la sélection :", 
            available_vars, 
            default=default_actives
        )
    ...
    with tab1: # Retour au tab 1 pour le calcul final et le bouton
        ...
        if st.button("LANCER L'ANALYSE", use_container_width=True):
            if len(vars_pca_actives) < 2:
                ...
                with st.spinner('Analyse et clustering en cours...'):
                    ...
                    if 'Poste' in df_filtered.columns: cols_to_keep.append('Poste')
                    cols_to_keep.extend(vars_pca_actives)
                    
                    df_analyse = df_filtered[cols_to_keep].copy().replace(r'^\s*$', np.nan, regex=True)
                    
                    # 2. Forcer les colonnes en numérique (les textes cachés deviennent des "NaN")
                    for col in vars_pca_actives: 
                        df_analyse[col] = pd.to_numeric(df_analyse[col], errors='coerce')
                    
                    # 3. SUPPRESSION STRICTE : On retire le joueur s'il manque au moins 1 donnée sur les tests choisis
                    df_analyse = df_analyse.dropna(subset=vars_pca_actives, how='any').reset_index(drop=True)
                    
                    # 4. Vérification s'il reste assez de joueurs pour créer les groupes
                    if len(df_analyse) >= n_clusters:
                        X = df_analyse[vars_pca_actives]
                        
                        # 5. Standardisation (Obligatoire pour l'ACP et le K-Means)
                        ...
                        # 8. Sauvegarde dans la session Streamlit
                        ...
                        st.error(f"Volume de données insuffisant : Il ne reste que {len(df_analyse)} joueur(s) ayant des données complètes pour ces tests.")
        ...
        if st.session_state.get('analyse_terminee', False):
            ...
            st.markdown("---")
            st.markdown(f"<h3 style='color:{SDR_RED};'>Recherche de Similarités (Distance Euclidienne Standardisée)</h3>", unsafe_allow_html=True)
            ...
            if joueur_cible:
                # On utilise l'espace complet standardisé (toutes les variables actives),
                # et non la projection ACP à 2 dimensions qui sert uniquement à la visualisation.
                # Cela garantit que la similarité calculée correspond bien à ce qui est affiché
                # ("Basé sur les X variables") et aux mêmes distances que celles utilisées par K-Means.
                ...
                if not df_top.empty:
                    ...
                    df_radar = pd.DataFrame(dict(
                        Métrique=vars_pca_actives, 
                        Cible=X_radar_visuel[idx_cible], 
                        Comparaison=X_radar_visuel[idx_comp],
                        Moyenne=X_radar_visuel.mean(axis=0)
                    ))
                    
                    df_radar = pd.concat([df_radar, df_radar.iloc[[0]]], ignore_index=True)
                    
                    fig_radar = go.Figure()
                    fig_radar.add_trace(go.Scatterpolar(r=df_radar['Cible'], theta=df_radar['Métrique'], fill='toself', name=joueur_cible, fillcolor='rgba(215, 25, 32, 0.4)', line=dict(color=SDR_RED, width=2), marker=dict(size=8)))
                    ...
                    fig_radar.add_trace(go.Scatterpolar(r=df_radar['Moyenne'], theta=df_radar['Métrique'], fill='none', name='Moyenne du groupe', line=dict(color='#888888', width=2, dash='dash'), marker=dict(size=6)))
