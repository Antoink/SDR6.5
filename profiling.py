...
def get_status_data_local(label, value):
    ...
    if "Ratio Squeeze" in col_clean:
        if 0.90 <= val <= 1.10:
            return "#27AE60", "Objectif Atteint", "#27AE60"
        else:
            return "#D71920", "Sous l'objectif", "#D71920"
    ...
    if isinstance(norm_val, (list, tuple)):
        ...
        if is_inverted(label):
            if val <= norm_val: return c_good, "Objectif Atteint", c_good
            else: return c_bad, "Sous l'objectif", c_bad
        else:
            if val >= norm_val: return c_good, "Objectif Atteint", c_good
            else: return c_bad, "Sous l'objectif", c_bad
...
""", unsafe_allow_html=True)
            ...
            for r_data in table_rows_data:
                ...
                if norm_txt == "-":
                    ...
                    if c_stat in ["#27AE60", "#00E5FF"]:
                        status_desc = "Objectif Atteint"
                        status_color = "#27AE60"
                    else:
                        status_desc = "Sous l'objectif"
                        status_color = "#D71920"
        ...
        def render_single_kpi(label, subtitle=None):
            ...
            # --- LOGIQUE ---
            if is_empty or pct is None or pd.isna(pct):
                pct_color = "#eee"
            ...
            if is_empty or not norm_txt_raw or norm_txt_raw == "()":
                norm_color = "#eee"
                status_text = "Pas d'objectif"
                norm_tooltip = "Aucun objectif défini"
            else:
