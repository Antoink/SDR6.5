...
from config_rapport import OFFICIAL_STRUCTURE, TEAM_STRUCTURE, REPORT_NORMES, UNITS, RELATIVE_NORM_KEYS
...
def is_inverted(label):
    keywords = ['temps', 'chrono', '10m', '505', 'agilité', 'masse grasse', 'landing', "Landing %", '1km', 
                'triceps', 'subscapulaire', 'biceps', 'iliaque', 'supraspinale', 'abdominal', 'cuisse', 'jambe', 'somme']
...
def get_unit(label):
    # On cherche d'abord la clé exacte (ex: "Nordic Ischio (G)"), car UNITS stocke
    # certaines entrées bilatérales AVEC le suffixe (G)/(D). Sans ce test, ces tests
    # ne trouvaient jamais leur unité et le mode relatif ne s'activait jamais pour eux.
    return UNITS.get(label, UNITS.get(get_clean_label(label), ""))
...
def get_norm_text(label, effective_mode=None, poids_joueur=None):
    ...
    norm_val = get_effective_norm(label, REPORT_NORMES[found_key], effective_mode, poids_joueur)
...
def get_status_data_local(label, value, effective_mode=None, poids_joueur=None):
...
def show_profiling_page(df_main=None):
    ...
    with tab_indiv:
        ...
        if sess_sel == "🏆 Record de Saison":
            from comparateur import get_best_season_record_paired
            ...
            row_dict['Equipe'] = df_player.iloc[-1].get('Equipe', 'N/A')
            row_dict['Poste'] = df_player.iloc[-1].get('Poste', df_player.iloc[-1].get('Position', 'N/A'))
            if 'Date' not in row_dict or pd.isna(row_dict.get('Date')):
                row_dict['Date'] = df_player.iloc[-1].get('Date', 'Bilan')
...
""", unsafe_allow_html=True)
        ...
        with c_table:
            ...
            for row_data in table_rows_data:
                ...
                date_html = f"<div style='font-size:10px; color:#aaa; font-style:italic; font-weight:normal; text-transform:none; margin-top:2px;'>{date_val}</div>" if date_val else ""
        ...
        def render_single_kpi(label, subtitle=None, mode_override=None):
            ...
            display_label = label
            if date_val:
                display_label = f"{label} <span style='float:right; font-size:10px; color:#aaa; font-weight:normal; font-style:italic; text-transform:none;'>{date_val}</span>"
        ...
        def render_pair_kpi(l_label, r_label, mode_override=None):
            ...
            def get_side_html(label, val, pct, unit, side_name, side_color):
                ...
                date_html = f"<span style='position:absolute; top:-2px; right:0px; font-size:9px; color:#aaa; font-style:italic; font-weight:normal;'>{date_val}</span>" if date_val else ""
...
<div style="text-align:right; font-size:12px; font-weight:bold; color:{col_n};">Meilleur que {int(pct_n)}% de l'équipe</div>
        ...
        def render_muscle_group_card(title_main, l_label, r_label, sum_label):
            ...
            dl_html = f" <span style='font-size:10px; color:#aaa; font-weight:normal; font-style:italic; margin-left:5px;'>{date_l}</span>" if date_l else ""
            
            date_r = format_date_str(row.get(f"{get_col_name(r_label)}_date", ""))
            dr_html = f" <span style='font-size:10px; color:#aaa; font-weight:normal; font-style:italic; margin-left:5px;'>{date_r}</span>" if date_r else ""

            date_s = format_date_str(row.get(f"{get_col_name(sum_label)}_date", ""))
            ds_html = f" <span style='font-size:11px; color:#aaa; font-weight:normal; font-style:italic; margin-left:8px;'>{date_s}</span>" if date_s else ""
...
# ==========================================
        ...
        with c_nutri1:
            ...
            if somme_plis > 0:
                ...
                fig_plis.update_layout(
                    polar=dict(radialaxis=dict(visible=True, gridcolor="#eee"), angularaxis=dict(tickfont=dict(size=11, weight="bold"), gridcolor="#eee")),
                    showlegend=False, height=350, margin=dict(l=40, r=40, t=20, b=20), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'
                )
                st.plotly_chart(fig_plis, use_container_width=True, config={'displayModeBar': False})
        ...
        with c_nutri2:
            ...
            if endo > 0 or meso > 0 or ecto > 0:
                ...
                st.plotly_chart(fig_soma, use_container_width=True, config={'displayModeBar': False})
            else:
        ...
        if val_cmj_dsi and val_squat_dsi and val_squat_dsi > 0:
            ...
            <div class="kpi-card" style="padding:15px; border-left:6px solid {dsi_color}; margin-top:20px;">
                <div style="font-size:14px; font-weight:900; color:#333; text-transform:uppercase;">DYNAMIC STRENGTH INDEX (DSI)</div>
                ...
                if sel_session_team == "🏆 Record de Saison":
                    ...
                    for j in df_team_view['Joueur'].dropna().unique():
                        ...
                        if not df_j.empty:
                            ...
                            # On s'assure que les métadonnées cruciales pour l'affichage d'équipe soient préservées
                            row_j['Joueur'] = j
                            row_j['Equipe'] = df_j.iloc[-1].get('Equipe', 'N/A')
                            row_j['Poste'] = df_j.iloc[-1].get('Poste', df_j.iloc[-1].get('Position', 'N/A'))
        ...
        with tab_classement:
             show_classement_page(pd.DataFrame())
      ...
      # 1. Récupérationdes notes ET des blessures
        ...
        row_report = row.copy()
        for k, v in report_records.items():
            row_report[k] = v

        html_rep = generate_report(
            p_sel, row_report, df, val_poste, val_lat, val_num, 
