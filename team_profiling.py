...
def show_team_page(df, structure_dict):
    ...
    if col_x_sc and col_y_sc:
        df_scatter = df.copy() # Correction : utilisation de df_main ou df pour cohérence
        if col_filter and sel_options:
            df_scatter = df_scatter[df_scatter[col_filter].isin(sel_options)]
        ...
        if not df_scatter.empty and len(df_scatter) > 1:
            ...
            corr_matrix = np.corrcoef(df_scatter['Val_X'], df_scatter['Val_Y'])
            r = corr_matrix[0, 1]
            r_squared = r**2
            ...
            # --- AFFICHAGE DU R ET R² ---
            # Interprétation rapide de la force de corrélation
    ...
    if col_dist:
        df_viz = df.copy()
        if col_filter and sel_options:
            df_viz = df_viz[df_viz[col_filter].isin(sel_options)]
